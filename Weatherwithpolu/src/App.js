...
class App extends React.Component {
  ...
  getWeather = async (e) => {
    ...
    const api_call2 = await fetch(`https://api.openweathermap.org/data/2.5/air_pollution?lat=${lat}&lon=${lon}&appid=${API_KEY}`)
    ...
    if(data2.list[0].main.aqi===1){
      ReactDOM.render("Air Quality : Good", document.getElementById("polusi"))    }
    else if(data2.list[0].main.aqi===2){
      ReactDOM.render("Air Quality : Fair", document.getElementById("polusi"))    }
    else if(data2.list[0].main.aqi===3){
      ReactDOM.render("Air Quality : Moderate", document.getElementById("polusi"))    }
    else if(data2.list[0].main.aqi===4){
      ReactDOM.render("Air Quality : Poor", document.getElementById("polusi"))    }
    else if(data2.list[0].main.aqi===5){
      ReactDOM.render("Air Quality : Very Poor", document.getElementById("polusi"))    }
      ...
      <div>
        ...
        <Form getWeather={this.getWeather} />
          <div className="col-6 isi">
            ...
            <p id="air" className="hidden">Air Quality Index: {this.state.aqi}</p>
            <p id="polusi" className="hidden"></p>
          </div>
          <br />
